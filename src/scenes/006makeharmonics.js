...
export default () => {
  ...
  const table = _.times(tableWaveCount, w => {
    return _.times(waveSampleCount, sample => {
      return _.sum(
        _.times(w + 1, harmonicIndex => {
          const harmonicSkip = 3
          const harmonic = harmonicIndex + 1 //* harmonicSkip
          const x = (2 * Math.PI * sample) / waveSampleCount
          const amp = 0.6 / harmonic
  ...
  fs.writeFile(`./waves/harmonics3.wav`, Buffer.concat(outFileBufferArray), function(e) {})
}
